...
- **Force low-bandwidth mode** — a switch that always plays Frigate's built-in low-bandwidth feed — the stream assigned the `detect` role, using JSMpeg — instead of the selected stream. It works anywhere without go2rtc and is useful on slow or metered connections. While it is enabled, the stream selection is disabled and the streaming technology selector is hidden; your stream and technology choices are restored when you turn it off.
