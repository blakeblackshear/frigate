...
- **`device`**: Device to use to run license plate recognition models.
  - Default: `AUTO`
  - This can be `CPU`, `GPU`, or `AUTO`. For users without a model that detects license plates natively, using a GPU may increase performance of the YOLOv9 license plate detector model.
