...
Birdseye RTSP restream can be accessed at `rtsp://<frigate_host>:8554/birdseye`. The restream will cause birdseye to run 24/7 which may increase CPU usage somewhat.
```yaml
birdseye:
  restream: true
```
