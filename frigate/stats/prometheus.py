...
class CustomCollector(object):
    ...
    def collect(self):
        ...
        for camera_name, camera_stats in cameras.items():
            ...
            self.add_metric(detection_enabled, [camera_name], camera_stats, 'detection_enabled')
        ...
        try:
            for detector_name, detector_stats in stats['detectors'].items():
                self.add_metric(detector_inference_speed, [detector_name], detector_stats, 'inference_speed',
                           0.001)  # ms to seconds
                self.add_metric(detector_detection_start, [detector_name], detector_stats, 'detection_start')
        ...
        try:
            for device_name in stats['service']['temperatures']:
                self.add_metric(temperatures, [device_name], stats['service']['temperatures'], device_name)
        except KeyError:
            pass
        ...
        try:
            for storage_path, storage_stats in stats['service']['storage'].items():
                self.add_metric(storage_free, [storage_path], storage_stats, 'free', 1e6)  # MB to bytes
                self.add_metric(storage_total, [storage_path], storage_stats, 'total', 1e6)  # MB to bytes
                self.add_metric(storage_used, [storage_path], storage_stats, 'used', 1e6)  # MB to bytes
