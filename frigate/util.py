...
def get_amd_gpu_stats() -> dict[str, str]:
    ...
    if p.returncode != 0:
        logger.error(f"Error polling radeon GPU stats: {p.stderr}")
        return None
    else:
...
def get_intel_gpu_stats() -> dict[str, str]:
    ...
    if p.returncode != 124:
        logger.error(f"Error polling intel GPU stats: {p.stderr}")
        return None
    else:
...
def get_nvidia_gpu_stats() -> dict[str, str]:
    ...
    if p.returncode != 0:
        logger.error(f"Error polling nvidia GPU stats: {p.stderr}")
        return None
    else:
        usages = p.stdout.split("\n")[1].strip().split(",")
