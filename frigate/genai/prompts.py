...
- Some images below are preceded by notes from the camera's object tracker recording what changed at that point: an object being first detected, starting to move, reversing direction, reaching the end of its movement, or no longer being detected. These notes come from tracking data rather than from the images, and they are reliable. Use them to establish how many distinct activities occur and in what order, and describe every one of them. The tracker opens a new entry each time a subject is re-detected, so unless two subjects of the same type are present at the same time, treat repeated entries as the same subject returning."""
...
FRAME_TIMELINE_GUIDANCE = """- The "Frame Notes" section at the end of this prompt lists, by frame number, notes from the camera's object tracker recording what changed at that point: an object being first detected, starting to move, reversing direction, reaching the end of its movement, or no longer being detected. The images follow in frame order, so note 1 describes the first image. These notes come from tracking data rather than from the images, and they are reliable. Use them to establish how many distinct activities occur and in what order, and describe every one of them. The tracker opens a new entry each time a subject is re-detected, so unless two subjects of the same type are present at the same time, treat repeated entries as the same subject returning."""
